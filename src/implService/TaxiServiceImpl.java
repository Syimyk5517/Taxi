...
public class TaxiServiceImpl implements TaxiService {
    ...
    public List<Taxi> findByInitialLetter(String model) {
        return database.getTaxis().stream().filter(x -> x.getModel().
                startsWith(model)).toList();
    }

    @Override
    public Map<TaxiType, List<Taxi>> grouping() {
        Map<TaxiType, List<Taxi>> taxiTypeListMap = database.getTaxis().stream().
    ...
    public List<Taxi> filterByTaxiType(String taxiType) {
        return database.getTaxis().stream().filter(x -> x.getTaxiType().
                equals(TaxiType.valueOf(taxiType))).toList();
    }
